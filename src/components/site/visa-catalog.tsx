...
import { visaPackages, visaFaqs, type VisaPackage } from "../../lib/data";
...
export function VisaCatalog({
  ...
  visaType: VisaPackage["visaType"];
  copy: VisaCatalogCopy;
}) {
  const scoped = useMemo(
    () => visaPackages.filter((v) => v.visaType === visaType),
    [visaType],
  );

  const countries = useMemo(() => {
    const unique = [...new Set(scoped.map((v) => v.country))].sort();
    return [ALL, ...unique];
  }, [scoped]);
  ...
  const filtered = useMemo(
    () => (selected === ALL ? scoped : scoped.filter((v) => v.country === selected)),
    [selected, scoped],
  );
  ...
  return (
    <div className="pt-32 pb-24">
      <div className="container-editorial">
        ...
        >
          <p className="text-eyebrow text-teal">{copy.eyebrow}</p>
          <h1 className="mt-3 font-display text-5xl sm:text-7xl leading-[1.02] text-navy">
            {copy.title}
          </h1>
          <p className="mt-6 text-xl leading-relaxed text-muted-foreground">{copy.intro}</p>
        </motion.div>
      </div>
